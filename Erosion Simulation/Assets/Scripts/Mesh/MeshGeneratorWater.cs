...
public static class MeshGeneratorWater
{
    public static Mesh GenerateMesh(float[,] noiseMap, float[,] waterMap)
    {
        ...
        for(int i=0;i<sizeX;i++)
        {
            ...
            {
                vertices[i * sizeY + j] = new Vector3(i, noiseMap[i, j] + waterMap[i,j], j);
                uvs[i * sizeY + j] = new Vector2(i / ((float)sizeX), j / ((float)sizeY));
